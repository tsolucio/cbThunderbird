...
var vtigerPreferences = Components.classes["@mozilla.org/preferences-service;1"].
							getService(Components.interfaces.nsIPrefService).getBranch("vtiger.");
...
function saveVtigerLoginDetails(){
	...
	var accessKey = trim(document.getElementById("txtpassword").value);
	username = encode64(username);
	accessKey = encode64(accessKey);
	var vtigerURL=trim(document.getElementById("txturl").value);
...
function getModuleEntity(moduleName){
	...
	for(var i=0; i<fieldList.length; i++) {
		...
		if(document.getElementById(fieldName).nodeName.toLowerCase() == 'checkbox'){
			...
			entity[fieldName] = trim(document.getElementById(fieldName).value);
		}
	}
	return entity;
}
...
function inArray(array,element){
	var i = 0;
	for ( var value = array[0];
			i < array.length && value !== element; value = array[++i] ){}
	if(i < array.length){
		return i;
	}
	return -1;

...
for ROOT_FILE in $ROOT_FILES manifest.json install.rdf chrome.manifest; do
...
# Generate hash of new xpi
echo "Generating hash for update.rdf ..."
sha1sum $APP_NAME.xpi > $APP_NAME.sha1
echo "Remember to add the hash to the update.rdf file if using McCoy!" 

echo "Moving files to the auto-update directory ..."
mv $APP_NAME.* auto-updates
echo "Done!"
